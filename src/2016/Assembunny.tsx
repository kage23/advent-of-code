      ...
      switch (currentInstruction[0]) {
        case 'cpy':
          if (this.registers.has(currentInstruction[2])) {
            if (isNaN(parseInt(currentInstruction[1]))) {
              this.registers.set(currentInstruction[2], this.registers.get(currentInstruction[1]) || 0)
            } else {
              this.registers.set(currentInstruction[2], parseInt(currentInstruction[1]))
            }
          }
          instructionPointer++
          break
        ...
        case 'inc':
          if (this.registers.has(currentInstruction[1])) {
            this.registers.set(currentInstruction[1], (this.registers.get(currentInstruction[1]) || 0) + 1)
          }
          instructionPointer++
          break
          ...
          if (isNaN(parsed)) {
            instructionPointer +=
              this.registers.get(currentInstruction[1]) === 0
                ? 1
                : this.registers.get(currentInstruction[2]) || parseInt(currentInstruction[2])
          } else {
            instructionPointer +=
              parsed === 0
                ? 1
        ...
        case 'tgl':
          const toToggle = instructionPointer + (this.registers.get(currentInstruction[1]) || NaN)
          if (!isNaN(toToggle)) {
              ...
              switch (instrToToggleWords.length) {
                case 2:
                  if (instrToToggleWords[0] === 'inc') {
                    instructions[toToggle] = ['dec', ...instrToToggleWords.slice(1)].join(' ')
                  } else {
                    instructions[toToggle] = ['inc', ...instrToToggleWords.slice(1)].join(' ')
                  }
                  break

                case 3:
                  if (instrToToggleWords[0] === 'jnz') {
                    instructions[toToggle] = ['cpy', ...instrToToggleWords.slice(1)].join(' ')
                  } else {
                    instructions[toToggle] = ['jnz', ...instrToToggleWords.slice(1)].join(' ')
                  }
                  break
