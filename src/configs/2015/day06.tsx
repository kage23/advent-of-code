...
const parseLightingInstruction = (
  lights: Map<string, boolean>,
  instruction: string
): Map<string, boolean> => {
  const instructionWords = instruction.split(' ')

  const [maxX, maxY] = instructionWords[instructionWords.length - 1]
    .split(',')
    .map((x) => parseInt(x))
  const [minX, minY] = instructionWords[instructionWords[0] === 'turn' ? 2 : 1]
    .split(',')
    .map((x) => parseInt(x))
...
const parseLightingInstruction__v2 = (
  lights: Map<string, number>,
  instruction: string
): Map<string, number> => {
  const instructionWords = instruction.split(' ')
  ...
  for (let x = minX; x <= maxX; x++) {
    for (let y = minY; y <= maxY; y++) {
      ...
      lights.set(
        `${x},${y}`,
        Math.max((lights.get(`${x},${y}`) || 0) + brightnessDiff, 0)
      )
    }
  }
...
export const findTotalBrightness = (inputKey: string) => {
  const lights: Map<string, number> = inputs
    ...
    .reduce(
      (lights, instruction) =>
        parseLightingInstruction__v2(lights, instruction),
      new Map()
    )

  return {
    answer2: Array.from(lights.values()).reduce(
