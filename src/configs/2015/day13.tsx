...
const calculateHappiness = (
  arrangement: string[],
  happinessLevels: Map<string, number>
): number => {
  ...
  for (let i = 0; i < arrangement.length; i++) {
    ...
    totalHappinessDiff +=
      happinessLevels.get(`${firstPerson}-${secondPerson}`) || 0
    totalHappinessDiff +=
      happinessLevels.get(`${secondPerson}-${firstPerson}`) || 0
  }
...
export const findOptimalSeatingArrangement = (inputKey: string) => {
  ...
  possibleSeatingArrangements.forEach((arrangement) => {
    highestPossibleHappiness = Math.max(
      highestPossibleHappiness,
      calculateHappiness(arrangement, happinessLevels)
    )
  })
...
const day13: Omit<DayConfig, 'year'> = {
  answer1Text:
    'The optimal seating arrangement generates a answer change in total happiness.',
  answer2Text:
    'The optimal seating arrangement including yourself generates a answer change in total happiness.',
  buttons: [
    {
