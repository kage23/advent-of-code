...
const move = (unit: Unit, field: Field) => {
  ...
  while (true) {
    ...
    for (const { nx, ny, value } of getAdjacentSquares(
      destX,
      destY,
      field,
      visited,
      ...
      queue.push([
        nx,
        ny,
        firstPlace ? nx : startX,
        firstPlace ? ny : startY,
        distance + 1,
      ])
    }
    firstPlace = false
  ...
  if (opponents.length > 0) {
    if (opponents.length > 1) opponents = sortOpponents(opponents)
  ...
  const opponent: Unit = units
    .filter(
      (u) =>
        u.type === opponentType &&
        // Adjacent check
        ((u.x === unit.x && (u.y === unit.y + 1 || u.y === unit.y - 1)) ||
          (u.y === unit.y && (u.x === unit.x + 1 || u.x === unit.x - 1)))
    )
    .sort(sortByReadingOrder)
    .reduce(
      (prev, current) => (current.hitPoints < prev.hitPoints ? current : prev),
...
const roundOfCombat = (
  ...
  return {
    ...
    field,
    rounds:
      roundAbandoned || (killedUnit === 'E' && breakOnElfDeath)
        ? rounds
        : rounds + 1,
...
const runCombat = (): State => {
  const { field, rounds, units } = state
...
const findElfPower = (state: State, input: string): State => {
...
export const doTheCombat = (input: string) => {
  state = reset(input.split('\n'))
  state = runCombat()
  const hitPointTotal = state.units.reduce(
...
export const findBestElfPower = (input: string) => {
  state = reset(input.split('\n'))
  state = findElfPower(reset(input.split('\n')), input)
  const hitPointTotal = state.units.reduce(
    (total, unit) => total + unit.hitPoints,
    0
  )
