...
export const findCorners = (input: string) => {
  ...
  const singletons = [...edgeList.entries()].filter(([, count]) => count === 1).map(([x]) => x)
  const corners = tiles.filter(({ edges }) => edges.filter(edge => singletons.includes(edge)).length === 2)

  return {
    answer1: corners.reduce((a, b) => a * b.id, 1)
  }
}
...
const createImage = (puzzle: Tile[][]): string[] => {
  const image: string[] = []
...
export const assemblePuzzle = (input: string) => {
  const placedTiles: number[] = []
  const tiles: Tile[] = input.split('\n\n').map(rawTile => {
    const tileLines = rawTile.split('\n')
    if (tileLines[tileLines.length - 1].length === 0) tileLines.pop()
    const edges: string[] = []
    const id = parseInt((tileLines.shift() as string).split('Tile ')[1])
  ...
  return {
    answer2: image.join('').split('').filter(x => x === '#').length
  }
}

const day20: Omit<DayConfig, 'year'> = {
  answer1Text: 'The product of the corner IDs is answer.',
  answer2Text: `The water's roughness is answer.`,
  buttons: [
    {
      label: 'Find Corners',
      onClick: findCorners
