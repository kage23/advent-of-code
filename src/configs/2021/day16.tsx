...
export const findPacketSum = (input: string) => {
...
export const getActualValue = (input: string) => {
  const bin = hexToBin(input)
  const outerPacket = decodeThePacket(bin)

  return {
    answer2: outerPacket.value
  }
}

const day16: Omit<DayConfig, 'year'> = {
  answer1Text: 'The version sum of the packet is answer.',
  answer2Text: 'The value of the outermost packet is answer.',
  buttons: [
    {
      label: 'Find Packet Sum',
      onClick: findPacketSum
    },
    {
      label: 'Get the Actual Value',
      onClick: getActualValue
    }
  ],
  id: 16,
  inputs,
